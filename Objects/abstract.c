...
{
    PyErr_Format(PyExc_TypeError, msg, Py_TP(obj)->tp_name);
    return NULL;
}
...
PyObject_GetItem(PyObject *o, PyObject *key)
{
    ...
    if (Py_TP(o)->tp_as_sequence) {
        ...
        else if (Py_TP(o)->tp_as_sequence->sq_item)
...
PyObject_SetItem(PyObject *o, PyObject *key, PyObject *value)
{
    ...
    if (Py_TP(o)->tp_as_sequence) {
        ...
        else if (Py_TP(o)->tp_as_sequence->sq_ass_item) {
...
{
    PyBufferProcs *pb = Py_TP(obj)->tp_as_buffer;
    Py_buffer view;
...
{
    return o && Py_TP(o)->tp_as_number &&
               (Py_TP(o)->tp_as_number->nb_int ||
	        Py_TP(o)->tp_as_number->nb_float);
}
...
binary_op1(PyObject *v, PyObject *w, const int op_slot)
{
    ...
    if (Py_TP(v)->tp_as_number != NULL)
        slotv = NB_BINOP(Py_TP(v)->tp_as_number, op_slot);
    if (Py_TP(w) != Py_TP(v) &&
        Py_TP(w)->tp_as_number != NULL) {
        slotw = NB_BINOP(Py_TP(w)->tp_as_number, op_slot);
        if (slotw == slotv)
            slotw = NULL;
    }
    if (slotv) {
        if (slotw && PyType_IsSubtype(Py_TP(w), Py_TP(v))) {
            x = slotw(v, w);
           ...
           const char *op_name)
{
    ...
    mv = Py_TP(v)->tp_as_number;
    mw = Py_TP(w)->tp_as_number;
    if (mv != NULL)
...
PyNumber_Multiply(PyObject *v, PyObject *w)
{
    ...
    if (result == Py_NotImplemented) {
        ...
        PySequenceMethods *mw = Py_TP(w)->tp_as_sequence;
        Py_DECREF(result);
        if  (mv && mv->sq_repeat) {
...
{
    PyNumberMethods *mv = Py_TP(v)->tp_as_number;
    if (mv != NULL) {
...
{
    if (Py_TP(v)->tp_as_number &&
        Py_TP(v)->tp_as_number->nb_inplace_power != NULL) {
...
PyNumber_Index(PyObject *item)
{
    ...
    if (!PyIndex_Check(item)) {
        PyErr_Format(PyExc_TypeError,
                     ...
                     "as an integer", Py_TP(item)->tp_name);
        return NULL;
    }
    result = Py_TP(item)->tp_as_number->nb_index(item);
    ...
    if (m && m->nb_float) { /* This should include subclasses of float */
        ...
        if (!PyFloat_Check(res)) {
            PyErr_Format(PyExc_TypeError,
                         ...
                         Py_TP(o)->tp_name, Py_TP(res)->tp_name);
            Py_DECREF(res);
            return NULL;
        }
...
PySequence_Check(PyObject *s)
{
    if (PyDict_Check(s))
        return 0;
    return Py_TP(s)->tp_as_sequence &&
        Py_TP(s)->tp_as_sequence->sq_item != NULL;
}
...
PySequence_Size(PyObject *s)
{
    ...
    if (Py_TP(s)->tp_as_mapping && Py_TP(s)->tp_as_mapping->mp_length) {
...
PySequence_GetItem(PyObject *s, Py_ssize_t i)
{
    ...
    if (Py_TP(s)->tp_as_mapping && Py_TP(s)->tp_as_mapping->mp_subscript) {
...
PySequence_SetItem(PyObject *s, Py_ssize_t i, PyObject *o)
{
    ...
    if (Py_TP(s)->tp_as_mapping && Py_TP(s)->tp_as_mapping->mp_ass_subscript) {
        type_error("%.200s is not a sequence", s);
        return -1;
    }
...
{
    Py_ssize_t result;
    PySequenceMethods *sqm = Py_TP(seq)->tp_as_sequence;
...
{
    return o && Py_TP(o)->tp_as_mapping &&
            Py_TP(o)->tp_as_mapping->mp_subscript;
}
...
PyMapping_Size(PyObject *o)
{
    ...
    if (Py_TP(o)->tp_as_sequence && Py_TP(o)->tp_as_sequence->sq_length) {
...
recursive_isinstance(PyObject *inst, PyObject *cls)
{
    ...
    if (PyType_Check(cls)) {
        ...
        if (retval == 0) {
            ...
            if (icls != NULL) {
                if (icls != (PyObject *)(Py_TP(inst)) && PyType_Check(icls)) {
...
{
    return Py_TP(obj)->tp_iternext != NULL &&
               Py_TP(obj)->tp_iternext != &_PyObject_NextNotImplemented;
}
...
{
    PyObject *result;
    result = (*Py_TP(iter)->tp_iternext)(iter);
    if (result == NULL &&
        PyErr_Occurred() &&
