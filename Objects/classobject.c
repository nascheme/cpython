...
method_getattro(PyObject *obj, PyObject *name)
{
    ...
    if (descr != NULL) {
        descrgetfunc f = TP_DESCR_GET(Py_TP(descr));
        if (f != NULL)
            return f(descr, obj, (PyObject *) Py_TP(obj));
        else {
            Py_INCREF(descr);
            return descr;
        }
...
instancemethod_getattro(PyObject *self, PyObject *name)
{
    ...
    if (descr != NULL) {
        descrgetfunc f = TP_DESCR_GET(Py_TP(descr));
        if (f != NULL)
            return f(descr, self, (PyObject *) Py_TP(self));
        else {
            Py_INCREF(descr);
            return descr;
        }
