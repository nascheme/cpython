...
do_richcompare(PyObject *v, PyObject *w, int op)
{
    ...
    if (Py_TP(v) != Py_TP(w) &&
        PyType_IsSubtype(Py_TP(w), Py_TP(v)) &&
        (f = Py_TP(w)->tp_richcompare) != NULL) {
    ...
    if ((f = Py_TP(v)->tp_richcompare) != NULL) {
        res = (*f)(v, w, op);
        if (res != Py_NotImplemented)
            return res;
        Py_DECREF(res);
    }
    if (!checked_reverse_op && (f = Py_TP(w)->tp_richcompare) != NULL) {
        res = (*f)(w, v, _Py_SwappedOp[op]);
        if (res != Py_NotImplemented)
            return res;
        Py_DECREF(res);
...
_PyObject_GetMethod(PyObject *obj, PyObject *name, PyObject **method)
{
    ...
    if (descr != NULL) {
        ...
        if (PyFunction_Check(descr) ||
            ...
            f = Py_TP(descr)->tp_descr_get;
            if (f != NULL && PyDescr_IsData(descr)) {
                *method = f(descr, obj, (PyObject *) Py_TP(obj));
                Py_DECREF(descr);
                return 0;
            }
        }
                                 ...
                                 PyObject *dict, int suppress)
{
    ...
    */
    ...
    if (descr != NULL) {
        ...
        f = Py_TP(descr)->tp_descr_get;
        if (f != NULL && PyDescr_IsData(descr)) {
            res = f(descr, obj, (PyObject *) Py_TP(obj));
...
PyObject_IsTrue(PyObject *v)
{
    ...
    if (v == Py_None)
        return 0;
    else if (Py_TP(v)->tp_as_number != NULL &&
             Py_TP(v)->tp_as_number->nb_bool != NULL)
        res = (*Py_TP(v)->tp_as_number->nb_bool)(v);
    else if (Py_TP(v)->tp_as_mapping != NULL &&
             Py_TP(v)->tp_as_mapping->mp_length != NULL)
        res = (*Py_TP(v)->tp_as_mapping->mp_length)(v);
    else if (Py_TP(v)->tp_as_sequence != NULL &&
             Py_TP(v)->tp_as_sequence->sq_length != NULL)
        res = (*Py_TP(v)->tp_as_sequence->sq_length)(v);
    else
        return 1;
