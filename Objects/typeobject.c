...
object_richcompare(PyObject *self, PyObject *other, int op)
{
    ...
    case Py_NE:
        ...
        if (Py_TP(self)->tp_richcompare == NULL) {
            res = Py_NotImplemented;
            Py_INCREF(res);
            break;
        }
        res = (*Py_TP(self)->tp_richcompare)(self, other, Py_EQ);
...
_PyObject_GetState(PyObject *obj, int required)
{
    ...
    if (getstate == NULL) {
        ...
        if (required && Py_TP(obj)->tp_itemsize) {
            PyErr_Format(PyExc_TypeError,
        ...
        if (required) {
            ...
            if (Py_TP(obj)->tp_dictoffset)
                basicsize += sizeof(PyObject *);
            if (Py_TP(obj)->tp_weaklistoffset)
                basicsize += sizeof(PyObject *);
            if (slotnames != Py_None)
                basicsize += sizeof(PyObject *) * PyList_GET_SIZE(slotnames);
            if (Py_TP(obj)->tp_basicsize > basicsize) {
                Py_DECREF(slotnames);
                Py_DECREF(state);
                PyErr_Format(PyExc_TypeError,
                             "can't pickle %.200s objects",
...
/*[clinic end generated code: output=73edab332f97d550 input=1200ff3dfe485306]*/
{
    ...
    res = 0;
    isize = Py_TP(self)->tp_itemsize;
    if (isize > 0)
        res = Py_SIZE(self) * isize;
    res += Py_TP(self)->tp_basicsize;
...
slot_sq_length(PyObject *self)
{
    ...
    if (_PyLong_NumDigits((PyLongObject*)res) < 0) {
        Py_DECREF(res);
        PyErr_SetString(PyExc_ValueError,
        ...
        if (set_name != NULL) {
            ...
            if (tmp == NULL) {
                _PyErr_FormatFromCause(PyExc_RuntimeError,
                    ...
                    "in '%.100s'",
                    Py_TP(value)->tp_name, key, type->tp_name);
